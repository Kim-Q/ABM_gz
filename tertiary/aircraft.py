...
class Aircraft(Agent):
    ...
    def receive(self, target_loc, target_activity):
        # print("I'm a aircraft with standpoint",str(self.standpoint),"receive the deployment of", target_activity)
        ...
        if target_activity=='fancheng':
            ...
            self.status=2
            self.wait=random.randint(1,10) #随机耗时10s左右
            ...
            self.status=3
            self.wait=random.randint(1,30) #随机耗时30s左右
            # print("work needs to be finished in", str(self.wait))
            self.target_loc.insert(0,target_loc)
            self.target_activity.insert(0,target_activity)
            target_loc=self.model.graph.nodes.data()[target_loc]['Lon_Lat']
    ...
    def step(self):
        # print("I'm a aircraft with standpoint",str(self.standpoint),"with statue", str(self.status))
        self.timestamp=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        if self.status==-1:
            return
